...
$solutionPath = Join-Path $projectRoot "HSRAchievementExporter.slnx"
$hookProject = Join-Path $projectRoot "src\HSRae.Hook\HSRae.Hook.csproj"
$appProject = Join-Path $projectRoot "src\HSRae.App\HSRae.App.csproj"
$versionSourcePath = Join-Path $projectRoot "src\HSRae.App\ApplicationBuildInfo.cs"
...
$hookBinary = Join-Path $hookOutput "HSRae.Hook.dll"
...
foreach ($currentConfiguration in $configurations) {
    ...
    if ($appFiles.Count -ne 1 -or $appFiles[0].Name -ne "HSRae.exe") {
        $names = $appFiles.Name -join ", "
        throw "Expected exactly one $currentConfiguration Host file named HSRae.exe, found: $names"
    }

    $assetName = "HSRae_v$($applicationVersion)_$currentConfiguration.exe"
...
$expectedNames = @(
    $configurations | ForEach-Object {
        "HSRae_v$($applicationVersion)_$($_).exe"
    }
)
...
$currentVersionNames = @(
    "HSRae_v$($applicationVersion)_Release.exe"
    "HSRae_v$($applicationVersion)_Debug.exe"
)
$managedAssetPattern = '^HSRae_v.+_(?:Release|Debug)\.exe$'
$oldBuildFiles = @(
