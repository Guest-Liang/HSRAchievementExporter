...
using HSRae.Formats.Backup;
using HSRae.Formats.Liyin;
using HSRae.Formats.Uiaf;
    ...
    )
    {
        ...
        switch (target)
        {
            case ExportTarget.AchievementBackup:
                fileName = $"HSRae-achievements-{stamp}.json";
                displayName = "成就数据备份";
                content = AchievementBackupExporter.Serialize(snapshot, uid, catalog.LatestVersion, catalog.Count);
                break;

            case ExportTarget.Liyin:
                fileName = $"HSRae-liyin-{stamp}.json";
                displayName = "由 HSRae 生成的 Liyin 格式";
                content = LiyinExporter.Serialize(snapshot, uid, catalog.Ids);
                break;

            case ExportTarget.UiafExperimental:
                fileName = $"HSRae-uiaf-{stamp}.json";
                displayName = "实验性 UIAF（非官方）";
                content = UiafExporter.Serialize(snapshot, uid, catalog.Ids);
                break;
