...
internal static class ExporterApplication
{
    ...
    {
        ApplicationLog.WriteInfo("HSRae — 崩坏：星穹铁道成就导出");
        ApplicationLog.WriteInfo("https://github.com/Guest-Liang/HSRAchievementExporter");
        Console.WriteLine();
        ...
        {
            ApplicationLog.WriteError("HSRae 只支持 Windows x64");
            return 2;
        }
    ...
    private static async Task<int> ExportAsync(string? configuredGamePath)
    {
        ...
        if (!ElevationManager.IsAdministrator())
        {
            ...
            ApplicationLog.WriteInfo($"游戏版本：{gameVersion}");
    ...
    private static bool TryParseArguments(string[] args, out string? configuredGamePath, out string? error)
    {
        ...
        {
            error = "无法识别命令行参数。HSRae 只支持可选参数 --game";
            return false;
        }

        if (args.Length == 1 || string.IsNullOrWhiteSpace(args[1]))
        {
            error = "--game 后必须提供游戏目录或 StarRail.exe 路径";
            return false;
        }

        if (args.Length != 2)
        {
            error = "--game 只能指定一个游戏目录或 StarRail.exe 路径";
            return false;
        }
    ...
    {
        ApplicationLog.WriteError(@"用法：HSRae.exe [--game ""游戏目录或 StarRail.exe 路径""]");
    }
    ...
    private static void EnsureGameIsNotRunning()
    {
        ...
        try
        {
            if (processes.Length != 0)
            {
                ...
                throw new InvalidOperationException("检测到崩坏：星穹铁道已经在运行，请先退出游戏，再运行 HSRae");
            }

            ApplicationLog.WriteDebug("游戏进程占用检查：未发现正在运行的 StarRail");
        }
        finally
        {
