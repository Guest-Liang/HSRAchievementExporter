...
/// </summary>
internal readonly record struct ParserLocation(nint Address, uint Rva, int PatchSize);

/// <summary>
/// 定位明文包解析器。
///
/// 这里不锚定入口机器码。函数序言里的寄存器搬运指令由编译器的寄存器分配决定，
/// 游戏重新编译一次就可能改变，固定字节特征会随版本失效。改为锚定两个更稳定的
/// 事实：
///
/// 1. 包头 <c>0x9D74C714</c> 和包尾 <c>0xD7A152C8</c> 是网络协议常量，由服务端和
///    客户端共同约定，不随客户端重新编译而改变；
...
/// </summary>
internal static unsafe class ParserLocator
{
    ...
    )
    {
        for (var sectionIndex = 0; sectionIndex < sectionCount; sectionIndex++)
        {
            ...
            var span = new ReadOnlySpan<byte>(image + virtualAddress, (int)Math.Min(safeSize, int.MaxValue));

            CollectOccurrences(span, virtualAddress, headPattern, headHits);
            CollectOccurrences(span, virtualAddress, tailPattern, tailHits);
        }
    }
